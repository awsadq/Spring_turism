...
public class AuthController {
    ...
    public String showLoginForm(WebRequest request) {
        return "login"; // Отображает страницу login.html
    }

    // Registration form endpoint
    @GetMapping("/register")
    public String showRegistrationForm(Model model) {
        model.addAttribute("user", new Person()); // Передает пустой объект Person в модель
        return "register"; // Отображает страницу register.html
    }
